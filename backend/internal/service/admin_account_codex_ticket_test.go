...
func TestUpdateAccountPreservesCodexTicketOnEdit(t *testing.T) {
	...
	for _, extra := range []map[string]any{
		{key: map[string]any{"model": ticket.Model, "length": 292}, "custom": true, OpenAICodexTicketEnabledExtraKey: false},
		{"custom": true, OpenAICodexTicketEnabledExtraKey: true},
	} {
		...
		require.Equal(t, extra[OpenAICodexTicketEnabledExtraKey], updated.Extra[OpenAICodexTicketEnabledExtraKey])
	}
...
func TestCreateAccountDropsUserSuppliedCodexTickets(t *testing.T) {
	account, err := buildAccountForCreate(&CreateAccountInput{Platform: PlatformOpenAI, Type: AccountTypeOAuth}, map[string]any{"codex_turn_ticket:custom": map[string]any{"state": "spoofed"}, "custom": true, OpenAICodexTicketEnabledExtraKey: false})
	require.NoError(t, err)
